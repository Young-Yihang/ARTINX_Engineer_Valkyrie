...
namespace SerialProtocol
{
    ...
    inline uint8_t Get_CRC8_Check_Sum(const uint8_t *pchMessage, uint16_t dwLength, uint8_t ucCRC8)
    {
        ...
        return Crc::Get_CRC8_Check_Sum(const_cast<uint8_t*>(pchMessage), static_cast<uint32_t>(dwLength), ucCRC8);
    }
    ...
    // CRC tables are centralized in Crc.{hpp,cpp}; avoid duplicating them here to ensure a single source of truth.
