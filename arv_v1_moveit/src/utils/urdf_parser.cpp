...
bool URDFDynamicsParser::parseURDF(const std::string& urdf_path) {
  ...
  std::cout << "[OK] KDL chain extracted: " << kdl_chain_.getNrOfSegments() << " segments, "
...
void URDFDynamicsParser::extractFromModel(const urdf::Model& model) {
  ...
  for (const auto& joint_name : joint_names) {
    ...
    if (link_ptr && link_ptr->inertial) {
      ...
      // 质心位置
      ld.com << link_ptr->inertial->origin.position.x, link_ptr->inertial->origin.position.y,
...
void URDFDynamicsParser::printSummary() const {
  ...
  for (size_t i = 0; i < joints_ordered_.size(); i++) {
    ...
    std::cout << "  [" << i + 1 << "] " << ji.name << " | type: " << ji.type << " | axis: ("
              << ji.axis.transpose() << ")" << " | torque_limit: " << ji.effort_limit << " N·m"
  ...
  for (size_t i = 0; i < links_ordered_.size(); i++) {
    ...
    std::cout << "  [" << i + 1 << "] " << ld.name << " | mass: " << ld.mass << " kg" << " | COM: ("
              << ld.com.transpose() << ") m" << std::endl;
  }
